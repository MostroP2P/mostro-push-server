...
- [x] **Phase 1: PushDispatcher refactor (no behaviour change)** — Extract the inline dispatch loop from the Nostr listener into a reusable `PushDispatcher` and drop the `Mutex<Vec<Box<dyn PushService>>>` in favour of `Arc<[Arc<dyn PushService>]>`.
- [x] **Phase 2: `POST /api/notify` endpoint with privacy hardening** — Ship the sender-triggered notify handler, request-id middleware, hash-based pubkey logging, and the deploy-side `RUST_LOG=info` change required to keep the endpoint privacy-safe in production.
- [x] **Phase 3: Dual-keyed rate limiting and verification harness** — Add per-IP `actix-governor` middleware (Fly-Client-IP keyed) scoped strictly to `/api/notify`, plus an in-handler per-`trade_pubkey` keyed limiter, with the integration-test suite that exercises the wiring end-to-end.
...
| 3. Dual-keyed rate limiting and verification harness | 2/2 | Complete | 2026-04-26 |

---

*Last updated: 2026-04-26 — Milestone shipped. All three phases complete: Phase 1 (1/1 plan), Phase 2 (3/3 plans), Phase 3 (2/2 plans). 17 / 17 requirements satisfied.*
